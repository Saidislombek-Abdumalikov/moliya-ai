...
/**
 * Client Helper: Upsert a transaction into public.transactions with authenticated JWT context.
 */
export async function writeTransactionRelationalClient(
  raw: any,
  userId: string,
  source: string = 'web'
  ...
  try {
    ...
    if (error) {
      console.error('[RELATIONAL_WRITER_CLIENT] Transaction write error:', error.message);
      return { success: false, error: error.message };
    }

    return { success: true, data };
  } catch (err: any) {
    console.error('[RELATIONAL_WRITER_CLIENT] Exception during transaction write:', err.message);
    return { success: false, error: err.message };
  }
}

/**
 * Client Helper: Delete a transaction from public.transactions.
 */
export async function deleteTransactionRelationalClient(
  txId: string,
  userId: string
): Promise<{ success: boolean; error?: string }> {
  if (!txId || !userId) {
    return { success: false, error: 'Missing txId or userId' };
  }
...
/**
 * Client Helper: Upsert a card into public.cards.
 */
export async function writeCardRelationalClient(
  raw: any,
  userId: string
  ...
  try {
    ...
    if (error) {
      console.error('[RELATIONAL_WRITER_CLIENT] Card write error:', error.message);
      return { success: false, error: error.message };
    }

    return { success: true, data };
  } catch (err: any) {
    console.error('[RELATIONAL_WRITER_CLIENT] Exception during card write:', err.message);
    return { success: false, error: err.message };
  }
}

/**
 * Client Helper: Delete a card from public.cards.
 */
export async function deleteCardRelationalClient(
  cardId: string,
  userId: string
): Promise<{ success: boolean; error?: string }> {
  if (!cardId || !userId) {
    return { success: false, error: 'Missing cardId or userId' };
  }
...
/**
 * Client Helper: Idempotent offline synchronization.
 * Upserts offline transactions and cards, and executes pending deletions,
 * without clobbering records created by Telegram or other devices.
 */
export async function syncOfflineDataRelationalClient(
  userId: string,
  localTxs: any[] = [],
  localCards: any[] = [],
  deletedTxIds: string[] = [],
  ...
  try {
    ...
    // 2. Upsert offline transactions (normalized, positive amount, valid cardId)
...
/**
 * Client Helper: Clear only financial data from relational tables (transactions & cards).
 */
export async function clearUserFinancialDataRelationalClient(
  userId: string
): Promise<{ success: boolean; error?: string }> {
